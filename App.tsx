...
const App: React.FC = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black text-blue-600 bg-white">লোড হচ্ছে...</div>;
...
const DashboardSummary: React.FC<{ user: UserProfile }> = ({ user }) => {
  ...
  const [stats, setStats] = useState({ totalExp: 0, totalSale: 0, totalWeight: 0 });
  const [activeTab, setActiveTab] = useState('overview');

  useEffect(() => {
    const fetchStats = async () => {
      const { data: exp } = await supabase.from('expenses').select('amount');
      const { data: sale } = await supabase.from('sales').select('amount, weight_kg');
      
      const totalExp = exp?.reduce((a, b) => a + Number(b.amount), 0) || 0;
      const totalSale = sale?.reduce((a, b) => a + Number(b.amount), 0) || 0;
      const totalWeight = sale?.reduce((a, b) => a + Number(b.weight_kg), 0) || 0;
      
      setStats({ totalExp, totalSale, totalWeight });
    };
    fetchStats();
  }, []);

  const profit = stats.totalSale - stats.totalExp;
  const breakEven = stats.totalWeight > 0 ? (stats.totalExp / stats.totalWeight).toFixed(2) : '০';

  const handleAddData = async () => {
    if (!calcItem || !calcWeight || !calcPrice) return alert('সকল তথ্য দিন');
    const table = calcType === 'expense' ? 'expenses' : 'sales';
    const totalAmount = calcWeight * calcPrice;
    
    const { error } = await supabase.from(table).insert([{
      user_id: user.id,
      amount: totalAmount,
      weight_kg: calcWeight,
      category: calcType === 'expense' ? 'ইনপুট হিসাব' : 'মাছ বিক্রয়',
      item_name: calcItem,
    ...
    if (!error) {
      ...
      setCalcItem(''); setCalcWeight(0); setCalcPrice(0);
      ...
      <div className="bg-white p-6 md:p-10 rounded-[2.5rem] shadow-2xl border border-blue-50 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
               <h3 className="text-2xl font-black text-slate-800">স্মার্ট ইনপুট এসিস্ট্যান্ট</h3>
               <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mt-1">দ্রুত খরচ ও বিক্রির হিসাব যোগ করুন</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-[0.1em]">মাছ/খাবার/পণ্য</label>
              <input value={calcItem} onChange={e => setCalcItem(e.target.value)} type="text" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl font-bold text-sm outline-none focus:ring-2 focus:ring-blue-500 shadow-inner" placeholder="নাম লিখুন..." />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-[0.1em]">পরিমাণ (কেজি)</label>
              <input value={calcWeight || ''} onChange={e => setCalcWeight(Number(e.target.value))} type="number" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl font-black text-sm outline-none focus:ring-2 focus:ring-blue-500 shadow-inner" placeholder="০০" />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-[0.1em]">দর (৳ প্রতি কেজি)</label>
              <input value={calcPrice || ''} onChange={e => setCalcPrice(Number(e.target.value))} type="number" className="w-full px-6 py-4 bg-slate-50 border-none rounded-2xl font-black text-sm outline-none focus:ring-2 focus:ring-blue-500 shadow-inner" placeholder="৳ ০০" />
            </div>
            <div className="flex items-center gap-3">
               <div className="flex-1 bg-blue-600/5 p-4 rounded-2xl border border-blue-100 flex flex-col items-center">
                  <p className="text-[8px] font-black text-blue-400 uppercase">মোট টাকা</p>
                  <p className="text-lg font-black text-blue-600">৳ {calcWeight * calcPrice}</p>
               </div>
               <button onClick={handleAddData} className={`p-5 rounded-2xl text-white shadow-xl hover:scale-105 active:scale-95 transition-all ${calcType === 'expense' ? 'bg-rose-500 shadow-rose-200' : 'bg-green-600 shadow-green-200'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className={`bg-white p-10 rounded-[3rem] shadow-sm border-t-8 transition-all hover:shadow-xl ${profit >= 0 ? 'border-green-500' : 'border-rose-500'}`}>
           <div className="flex justify-between items-start mb-8">
              ...
              <span className={`text-2xl ${profit >= 0 ? 'text-green-500' : 'text-rose-500'}`}>{profit >= 0 ? '📈' : '📉'}</span>
           </div>
           ...
           <p className="mt-4 text-xs font-bold text-slate-400 flex items-center gap-2">
              {profit >= 0 ? (
                ...
                <><span className="w-2 h-2 bg-rose-500 rounded-full animate-pulse"></span> খরচ বিক্রির চেয়ে বেশি</>
              )}
           </p>
        </div>
        ...
        <div className="bg-slate-900 p-10 rounded-[3rem] shadow-2xl text-white relative overflow-hidden group">
           ...
           <div className="relative z-10">
              ...
              <p className="mt-6 text-xs font-medium text-slate-400 leading-relaxed italic">
                 "মুনাফা করতে হলে আপনাকে প্রতি কেজি মাছ <span className="text-blue-400 font-black">৳ {breakEven}</span> এর চেয়ে বেশি দামে বিক্রি করতে হবে।"
              </p>
           </div>
        ...
        <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 flex flex-col justify-between">
           <div>
              ...
              <div className="flex items-center gap-4 mb-4">
                 <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 text-2xl">👑</div>
                 <div>
                    <p className="font-black text-slate-800">{user.max_ponds === 999 ? 'আনলিমিটেড' : user.max_ponds + 'টি'} পুকুর প্যাকেজ</p>
                    <p className="text-[10px] text-green-600 font-bold uppercase">সক্রিয় সদস্য</p>
                 </div>
              </div>
           </div>
