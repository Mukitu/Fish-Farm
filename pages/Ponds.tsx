...
const PondsPage: React.FC<{ user: UserProfile }> = ({ user }) => {
  ...
  const handleAddPond = async () => {
    if (ponds.length >= user.max_ponds) {
      alert(`⚠️ আপনার প্যাকেজ লিমিট পূর্ণ! সর্বোচ্চ ${user.max_ponds}টি পুকুর সম্ভব।`);
      return;
    }
    setSaving(true);
    try {
  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-black text-slate-800 tracking-tight">পুকুরসমূহ</h1>
        ...
        {ponds.map(pond => (
          <div key={pond.id} className="bg-white rounded-[3rem] shadow-sm border border-slate-100 p-10 hover:shadow-2xl transition-all">
            <div className="flex justify-between mb-6">
              ...
              <button onClick={async () => {if(confirm('ডিলিট?')) {await supabase.from('ponds').delete().eq('id', pond.id); fetchPonds();}}} className="text-slate-200 hover:text-rose-500">🗑️</button>
            </div>
            ...
            <div className="bg-slate-50 p-6 rounded-[2rem] space-y-4 mb-8">
              <div className="flex justify-between text-sm font-bold">
                <span className="text-slate-400">মাছ:</span> 
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 z-50">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 space-y-6">
            <h3 className="text-2xl font-black text-center text-slate-800">নতুন পুকুর</h3>
            ...
            <div className="flex gap-4">
              ...
              <button onClick={handleAddPond} disabled={saving} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black">{saving ? '...' : 'সেভ'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isStockModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6 z-50">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 space-y-6">
            <h3 className="text-2xl font-black text-center text-slate-800">মাছ মজুদ</h3>
            <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <button onClick={handleStocking} disabled={saving} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black">{saving ? '...' : 'মজুদ করুন'}</button>
            </div>
          </div>
        </div>
      )}
