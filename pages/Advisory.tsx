...
const AdvisoryPage: React.FC<{ user: UserProfile }> = ({ user }) => {
  ...
  const calculatePlan = () => {
    ...
    guideMd += `#### 📊 পোনা মজুদের সারসংক্ষেপ\n`;
    guideMd += `- **রুই:** ${Math.round(areaVal * 9)} টি\n`;
    guideMd += `- **কাতলা:** ${Math.round(areaVal * 3)} টি\n`;
    guideMd += `- **মৃগেল:** ${Math.round(areaVal * 5)} টি\n`;
    guideMd += `- **সিলভার কার্প:** ${Math.round(areaVal * 3)} টি\n`;
    guideMd += `- **মোট পোনা:** ${Math.round(areaVal * 20)} টি\n`;
    guideMd += `- **পোনার গড় ওজন:** ৫-১০ গ্রাম\n`;
    guideMd += `- **প্রত্যাশিত উৎপাদন:** ${Math.round(areaVal * 16)} কেজি (${months} মাসে)\n`;
    guideMd += `- **আনুমানিক আয়:** ${(Math.round(areaVal * 16) * 175).toLocaleString()} - ${(Math.round(areaVal * 16) * 200).toLocaleString()} টাকা\n\n`;

    for (let i = 1; i <= months; i++) {
      guideMd += `#### 📅 মাস ${i}: ${i === 1 ? 'পুকুর প্রস্তুতি ও পোনা মজুদ' : (i === months ? 'মাছ আহরণ ও বাজারজাতকরণ' : 'মাছের বৃদ্ধি পর্যায়')}\n`;
      if (i === 1) {
        guideMd += `> ⚠️ **সতর্কতা:** অতিরিক্ত গরমে পানির স্তর কমে যেতে পারে, পানি সরবরাহ নিশ্চিত করুন।\n\n`;
        guideMd += `- **পুকুর প্রস্তুতি:** চুন **${(areaVal * 1).toFixed(1)} কেজি** (পুকুর শুকানোর পর পানিতে গুলে ছিটিয়ে দিন)। গোবর সার **${(areaVal * 4).toFixed(1)} কেজি** (চুন দেওয়ার ৩-৪ দিন পর)।\n`;
        guideMd += `- **পোনা মজুদ:** মিশ্র পোনা **${Math.round(areaVal * 20)} টি** (ভোর বেলা বা পড়ন্ত বিকেলে লবণ পানিতে ৫ মিনিট স্নান করিয়ে ছাড়ুন)।\n`;
        guideMd += `- **সার প্রয়োগ:** ইউরিয়া **${(areaVal * 0.08).toFixed(1)} কেজি**, টিএসপি **${(areaVal * 0.04).toFixed(1)} কেজি** (পোনা ছাড়ার ৭ দিন পর)।\n`;
        guideMd += `- **খাদ্য ব্যবস্থাপনা:** সম্পূরক খাবার মাছের মোট ওজনের ৩-৫% (সকাল ৮টা ও বিকেল ৪টা)।\n`;
        guideMd += `- **রোগ প্রতিরোধ:** মাসের শেষ সপ্তাহে লবণ **${(areaVal * 0.2).toFixed(1)} কেজি**।\n\n`;
      } else if (i === months) {
        guideMd += `> ⚠️ **সতর্কতা:** আহরণের আগে মাছের স্বাস্থ্য পরীক্ষা করুন এবং বাজার দর যাচাই করুন।\n\n`;
        guideMd += `- **মাছ আহরণ:** ভোরবেলা বা সকালে বড় ফাঁসের জাল দিয়ে ধাপে ধাপে বড় মাছগুলো তুলে ফেলুন।\n`;
        guideMd += `- **খাবার বন্ধ করা:** মাছ ধরার ১-২ দিন আগে খাবার দেওয়া সম্পূর্ণ বন্ধ রাখুন।\n\n`;
      } else {
        guideMd += `> ⚠️ **সতর্কতা:** ${i % 2 === 0 ? 'তাপমাত্রা বেশি থাকায় ফাইটোপ্লাংকটন ব্লুম হতে পারে, পানির রঙের দিকে খেয়াল রাখুন।' : 'বৃষ্টির কারণে পানির pH কমে যেতে পারে, চুন প্রয়োগ জরুরি।'}\n\n`;
        guideMd += `- **সার প্রয়োগ:** ইউরিয়া **${(areaVal * 0.08).toFixed(1)} কেজি**, টিএসপি **${(areaVal * 0.04).toFixed(1)} কেজি** (মাসের মাঝামাঝি সময়ে)।\n`;
        guideMd += `- **খাদ্য ব্যবস্থাপনা:** সম্পূরক খাবার মাছের মোট ওজনের ৩-৪% (সকাল ও বিকেল)।\n`;
        guideMd += `- **নমুনা সংগ্রহ:** মাসের শেষে জাল টেনে কিছু মাছ ধরে ওজন পরীক্ষা করুন।\n`;
        if (i % 2 !== 0) {
          guideMd += `- **রোগ প্রতিরোধ:** চুন **${(areaVal * 0.25).toFixed(1)} কেজি**, লবণ **${(areaVal * 0.2).toFixed(1)} কেজি** (মাসের যেকোনো দিন)।\n\n`;
        } else {
          guideMd += `- **গ্যাস দূরীকরণ:** জিওলাইট **${(areaVal * 0.5).toFixed(1)} কেজি** (পুকুরের তলায় গ্যাস হলে)।\n\n`;
        }
      }
    }

    guideMd += `#### 🎣 আহরণ পরিকল্পনা (Harvest Plan)\n`;
    guideMd += `- **আহরণের সময়:** ${months} তম মাস\n`;
    guideMd += `- **পদ্ধতি:** বড় ফাঁসের জাল দিয়ে ধাপে ধাপে বড় মাছগুলো আগে আহরণ করুন।\n`;
    guideMd += `- **পরবর্তী করণীয়:** মাছ বিক্রির পর পুকুর শুকিয়ে বা রোটেনন দিয়ে রাক্ষুসে মাছ মেরে, চুন দিয়ে পরের মৌসুমের জন্য প্রস্তুত করুন।\n\n`;

    guideMd += `#### ⚠️ সাধারণ নিয়মাবলী (General Rules)\n`;
    guideMd += `- চুন ও সার একসাথে প্রয়োগ করবেন না — ৭ দিন ব্যবধান রাখুন।\n`;
    guideMd += `- ভোরবেলা মাছ ভেসে উঠলে অক্সিজেন সংকট — সাথে সাথে এ্যারেটর চালু করুন বা পানি নাড়াচাড়া করুন।\n`;
    guideMd += `- প্রতি ১৫ দিনে জাল দিয়ে স্যাম্পলিং করুন মাছের বৃদ্ধি দেখতে।\n`;
    guideMd += `- বৃষ্টির পরদিন অবশ্যই পুকুরে চুন দিন।\n`;
    guideMd += `- মরা মাছ দেখলে সাথে সাথে তুলে ফেলুন ও উপজেলা মৎস্য কর্মকর্তার পরামর্শ নিন।\n\n`;
